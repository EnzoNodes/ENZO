UNIX BUILD NOTES
====================
Some notes on how to build ENZO in Unix.

Note
---------------------
Always use absolute paths to configure and compile ENZO and the dependencies,
...
This will build enzo-qt as well, if the dependencies are met.
...
memory available when compiling ENZO. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~enzo/+archive/enzo).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:enzo/enzo
    sudo apt-get update
...
To build ENZO without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build enzo-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a enzo-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip enzod" to strip the debug
...
Security
--------
To help make your ENZO installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, ENZO should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, ENZO may be compiled in
