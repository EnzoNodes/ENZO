...
*Setup instructions for a gitian build of ENZO using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ENZO
...
to the ENZO GitHub release page.
...
- [Building ENZO](#building-enzo-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/enzo-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/enzo-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/enzo-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/enzo-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/enzo-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/enzo-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/enzo-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@enzo.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/enzo-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/enzo-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/enzo-osx-*-build.assert
```
