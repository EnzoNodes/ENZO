...
class ReorgStakeTest(enzoTestFramework):
    ...
    def check_money_supply(self, expected_enz):
        # verify that nodes have the expected ENZ supply
        enz_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(enz_supply, [DecimalAmt(expected_enz)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that ENZ supply was properly updated after the reorgs
        self.log.info("Check ENZ supply...")
