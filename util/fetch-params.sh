...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ENZOParams"
    else
        PARAMS_DIR="$HOME/.enzo-params"
    fi
fi
...
Failed to fetch the ENZO zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
ENZO - fetch-params.sh

This script will fetch the ENZO zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ENZO zkSNARK parameters. Note that it is
