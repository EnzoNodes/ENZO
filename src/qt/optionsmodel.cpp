...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ENZ);
    ...
    if (settings.contains("nAnonymizeenzoAmount") || reset)
        gArgs.SoftSetArg("-anonymizeenzoamount", settings.value("nAnonymizeenzoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in enzo.cpp during shotdown to also remove the window positions
