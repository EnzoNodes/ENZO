...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select ENZ Outputs to Spend" :
            "Select Shielded ENZ to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::ENZ;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show ENZO address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
