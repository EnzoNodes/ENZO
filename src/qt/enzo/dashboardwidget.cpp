...
#include "qt/enzo/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/enzo/forms/ui_dashboardwidget.h"
#include "qt/enzo/qtutils.h"
#include "qt/enzo/sendconfirmdialog.h"
#include "qt/enzo/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(ENZOGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareEnz, "square-chart-enz");
    setCssProperty(ui->labelEnz, "text-chart-enz");
    ...
    setCssProperty(ui->labelAmountEnz, "text-stake-enz-disable");
    ...
    if (window)
        connect(window, &ENZOGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ENZ")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(41, 73, 106);
        linePenColorY = gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ENZ
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            enz = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalEnz += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEnz.append(enz);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(87, 158, 230));
    set1->setColor(QColor(87, 158, 230));
    ...
    set0->append(chartData->valuesEnz);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEnz > 0) {
        setCssProperty(ui->labelAmountEnz, "text-stake-enz");
    } else {
        setCssProperty(ui->labelAmountEnz, "text-stake-enz-disable");
    }
    forceUpdateStyle({ui->labelAmountEnz});
    ui->labelAmountEnz->setText(GUIUtil::formatBalance(chartData->totalEnz, nDisplayUnit));
