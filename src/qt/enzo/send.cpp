...
#include "qt/enzo/send.h"
#include "qt/enzo/forms/ui_send.h"
#include "qt/enzo/addnewcontactdialog.h"
#include "qt/enzo/qtutils.h"
#include "qt/enzo/sendchangeaddressdialog.h"
#include "qt/enzo/optionbutton.h"
#include "qt/enzo/sendconfirmdialog.h"
#include "qt/enzo/guitransactionsutils.h"
...
SendWidget::SendWidget(ENZOGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-enz");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onENZSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onENZSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent ENZs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield ENZs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent ENZs to shield."));
    }
}
...
void SendWidget::onENZSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto enzAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(enzAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(enzAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(enzAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
