...
#include "qt/enzo/forms/ui_snackbar.h"
#include "qt/enzo/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ENZOGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ENZOGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
