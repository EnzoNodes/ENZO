...
#include "qt/enzo/settings/settingsfaqwidget.h"
#include "qt/enzo/qtutils.h"
#include "qt/enzo/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(ENZOGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableENZ,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableENZ,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("ENZO is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. ENZO incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("ENZO utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of ENZO to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableENZContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received ENZO requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12-15 minutes.")) +
        formatFAQParagraph(
            tr("Your ENZO wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableENZ->setText(unspendableENZContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of ENZO with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("enzo", "<span style=\"font-style:italic\">getstakingstatus</span>")));

    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For Support you can contact us on our Email: info@enzocoin.io")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node ENZO wallet with a "
               "requirement of Collateral Amount of ENZ secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in ENZO Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the ENZO network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Collateral Amount of ENZ per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the Collateral Amount of ENZ collateral "
    ...
    connect(ui->pushButton_UnspendableENZ, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableENZ); });
    ...
    if (parent)
        connect(parent, &ENZOGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
