...
#include "qt/enzo/forms/ui_addresseswidget.h"
#include "qt/enzo/addresslabelrow.h"
#include "qt/enzo/addnewaddressdialog.h"
#include "qt/enzo/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(ENZOGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto enzAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(enzAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(enzAdd);
        if (walletModel->updateAddressBookLabels(enzAdd, label.toUtf8().constData(),
