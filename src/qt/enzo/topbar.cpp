...
#include "qt/enzo/topbar.h"
#include "qt/enzo/forms/ui_topbar.h"
#include "qt/enzo/lockunlock.h"
#include "qt/enzo/qtutils.h"
#include "qt/enzo/receivedialog.h"
#include "qt/enzo/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/enzo/balancebubble.h"
...
TopBar::TopBar(ENZOGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitle3, ui->labelTitle4, ui->labelTrans, ui->labelShield,
        ui->labelMasternodesTitle, ui->labelTitle8, ui->labelNextMasternodesTitle, ui->labelTitle9};
    ...
    setCssProperty({ui->labelAmountTopEnz, ui->labelAmountTopShieldedEnz}, "amount-small-topbar");
    setCssProperty({ui->labelAmountEnz}, "amount-topbar");
    setCssProperty({ui->labelPendingEnz, ui->labelImmatureEnz, ui->labelCollateralEnz, ui->labelMasternodeCount,
                    ui->labelNextCollateralBlocks , ui->labelNextCollateralValue}, "amount-small-topbar");
    ...
    ui->pushButtonConf->setButtonText("enzo.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open enzo.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ENZO")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralEnz->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // ENZ Total
    QString totalEnz = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // ENZ
    // Top
    ui->labelAmountTopEnz->setText(totalTransparent);
    ui->labelAmountTopShieldedEnz->setText(totalShielded);
    // Expanded
    ui->labelAmountEnz->setText(totalEnz);
    ui->labelPendingEnz->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureEnz->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
