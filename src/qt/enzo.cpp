...
#include "qt/enzo/splash.h"
#include "qt/enzo/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("enzo", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in enzo.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in enzo.qrc)
...
/** Class encapsulating ENZO startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (ENZOGUI)
...
{
    window = new ENZOGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ENZOGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ENZOGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ENZOGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ENZOGUI::DEFAULT_WALLET);
        ...
        // ENZO: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ENZOGUI::handlePaymentRequest);
        connect(window, &ENZOGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. ENZO can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(enzo_locale);
    Q_INIT_RESOURCE(enzo);
    ...
    /// 6. Determine availability of data directory and parse enzo.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("ENZO"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("ENZO"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // enzo: links repeatedly have their payment requests routed to this process:
