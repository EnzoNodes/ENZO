...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 10921) return false;
    } else if (addr.GetPort() == 10921)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int nChainHeight, int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ENZ tx got MASTERNODE_MIN_CONFIRMATIONS
...
{
    if (nHeight > 0 && nHeight <= 50000) {
        return std::make_pair(50000 - nHeight, 11000 * COIN);
    } else if (nHeight > 50000 && nHeight <= 125000) {
        return std::make_pair(125000 - nHeight, 12000 * COIN);
    } else if (nHeight > 125000 && nHeight <= 225000) {
        return std::make_pair(225000 - nHeight, 13000 * COIN);
    } else if (nHeight > 225000 && nHeight <= 350000) {
        return std::make_pair(350000 - nHeight, 14000 * COIN);
    } else if (nHeight > 350000 && nHeight <= 500000) {
        return std::make_pair(500000 - nHeight, 15000 * COIN);
    } else if (nHeight > 500000 && nHeight <= 675000) {
        return std::make_pair(675000 - nHeight, 16000 * COIN);
    } else if (nHeight > 675000 && nHeight <= 875000) {
        return std::make_pair(875000 - nHeight, 18000 * COIN);
    } else if (nHeight > 875000 && nHeight <= 1100000) {
        return std::make_pair(1100000 - nHeight, 20000 * COIN);
    } else {
        return std::make_pair(0, 20000 * COIN);
    }
