...
CEnzStake* CEnzStake::NewEnzStake(const CTxIn& txin)
{
    ...
    return new CEnzStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CEnzStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CEnzStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CEnzStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CEnzStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CEnzStake::GetUniqueness() const
{
    //The unique identifier for a ENZ stake is the outpoint
...
const CBlockIndex* CEnzStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CEnzStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CEnzStake::ContextCheck(int nHeight, uint32_t nTime)
{
