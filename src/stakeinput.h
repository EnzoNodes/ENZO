...
#ifndef ENZO_STAKEINPUT_H
#define ENZO_STAKEINPUT_H
...
class CEnzStake : public CStakeInput
{
private:
...
public:
    CEnzStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CEnzStake* NewEnzStake(const CTxIn& txin);
