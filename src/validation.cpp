...
#if defined(NDEBUG)
#error "ENZO cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uenz) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
	...
	else if (nHeight >= nLast && nHeight <= 50000) {
		return 23 * COIN;
	}
	else if (nHeight > 50000 && nHeight <= 125000) {
		return 21.9 * COIN;
	}
	else if (nHeight > 125000 && nHeight <= 225000) {
		return 20.8 * COIN;
	}
	else if (nHeight > 225000 && nHeight <= 350000) {
		return 19.7 * COIN;
	}
	else if (nHeight > 350000 && nHeight <= 500000) {
		return 18.6 * COIN;
	}
	else if (nHeight > 500000 && nHeight <= 675000) {
		return 17.5 * COIN;
	}
	else if (nHeight > 675000 && nHeight <= 875000) {
		return 16.4 * COIN;
	}
	else if (nHeight > 875000 && nHeight <= 1100000) {
		return 15.3 * COIN;
	}
	else {
		return 12 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	...
	if (nHeight > 0 && nHeight < nLast) {
		...
		ret = blockValue * 0.8696;  // 86.96%
	}
	else if (nHeight > 50000 && nHeight <= 125000) {
		ret = blockValue * 0.8676;  // 86.76%
	}
	else if (nHeight > 125000 && nHeight <= 225000) {
		ret = blockValue * 0.8654;  // 86.54%
	}
	else if (nHeight > 225000 && nHeight <= 350000) {
		ret = blockValue * 0.8629;  // 86.29%
	}
	else if (nHeight > 350000 && nHeight <= 500000) {
		ret = blockValue * 0.8602;  // 86.02%
	}
	else if (nHeight > 500000 && nHeight <= 675000) {
		ret = blockValue * 0.8571;  // 85.71%
	}
	else if (nHeight > 675000 && nHeight <= 875000) {
		ret = blockValue * 0.8537;  // 85.37%
	}
	else if (nHeight > 875000 && nHeight <= 1100000) {
		ret = blockValue * 0.8497;  // 84.97%
	}
	else {
		ret = blockValue * 0.8333; // 83.33%
	}
...
{
    util::ThreadRename("enzo-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> enzInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                enzInputs.push_back(stakeIn);
        }
        const bool hasENZInputs = !enzInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasENZInputs) {
                    ...
                    for (const CTxIn& enzIn : enzInputs)
                        if(enzIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasENZInputs) {
                            for (const CTxIn& stakeIn : enzInputs)
